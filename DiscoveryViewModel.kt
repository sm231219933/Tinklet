...
// Final fallback: fetch directly from the public profile endpoint.
                            ...
                            data.matches.forEach { match ->
                                ...
                                if (!partnerEmail.isNullOrBlank()) {
                                    try {
                                        // 1. Server se real profile fetch karein
                                        val remoteRes = RetrofitClient.apiService.getProfileSecure(email = partnerEmail)

                                        if (remoteRes.isSuccessful) {
                                            // 2. Sahi tareeqa: Body ko 'let' se use karein (Kyuki 'user' field nahi hai)
                                            remoteRes.body()?.let { profile ->
                                                if (!profile.isMe) {
                                                    profileDao.insertProfiles(
                                                        listOf(profile.copy(
                                                            email = partnerEmail,
                                            ...
                                            Log.e("CloudSync", "Match sync failed for $partnerEmail: ${remoteRes.code()}")
                                        }
                                    } catch (e: Exception) {
                                        Log.e("CloudSync", "Failed to fetch match profile: $partnerEmail", e)
                                    }
                                }
